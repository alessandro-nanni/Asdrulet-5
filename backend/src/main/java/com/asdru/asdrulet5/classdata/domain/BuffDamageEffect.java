...
public record BuffDamageEffect(int power, int durationTurns) implements AbilityEffect {
    public BuffDamageEffect {
        if (power <= 0) {
    ...
    public void apply(EffectTarget actor, EffectTarget target) {
        target.addActiveEffect(BuffKind.DAMAGE, power, durationTurns);
    }
}
