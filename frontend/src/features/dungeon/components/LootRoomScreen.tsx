  ...
  return (
    <Portal>
      <div className="loot-screen">
        ...
        {!hasLooted && isMyTurn && (
          <button type="button" className="btn btn-primary btn-block" onClick={handleOpen} disabled={isOpening}>
          ...
          {members.map((member) => {
            ...
            return (
              <li key={member.userId} className={`loot-member-row${isNext ? ' is-current-turn' : ''}`}>
                ...
                <span className="muted">{result ? summarize(result) : isNext ? 'Looting now…' : 'Waiting to loot…'}</span>
              </li>
            )
          })}
        </ul>
